...
DROP VIEW IF EXISTS not_alive_cow;
CREATE VIEW not_alive_cow AS
SELECT *
FROM cow
WHERE alive = 0;
...
DROP VIEW IF EXISTS female_cow;
CREATE VIEW female_cow AS
SELECT *
FROM cow
WHERE gender = 'HEMBRA';
...
DROP VIEW IF EXISTS vaccine_cow_view;
CREATE VIEW vaccine_cow_view AS
...
DROP VIEW IF EXISTS descriptive_lotcow_view;
CREATE VIEW descriptive_lotcow_view AS 
SELECT lc.id, l.id_lot, c.cow_name, l.lot_address 
FROM cow c, lotcow lc, lot l
WHERE lc.id_lot = l.id_lot AND lc.id_cow = c.id_cow;

DROP VIEW IF EXISTS breed_lotcow_view;
CREATE VIEW breed_lotcow_view AS 
SELECT lc.id_cow,lc.id_lot, lc.cow_name, lc.cow_desc, bc.breed_name, lc.lot_address
FROM lotcow_view lc
