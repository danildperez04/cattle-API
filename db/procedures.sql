...
DELIMITER $$
CREATE FUNCTION DEADCOW_EXISTS(id_cow INT) RETURNS BOOLEAN DETERMINISTIC -- RETORNA VERDADERO EN CASO DE QUE LA RAZA EXISTA
BEGIN
	DECLARE id INT;
	SELECT d.id_cow FROM deadcow d WHERE d.id_cow = id_cow INTO id;
    IF id IS NULL THEN
		RETURN FALSE;
    END IF;
	RETURN TRUE;
...
DELIMITER ;; 
CREATE PROCEDURE GET_DEADCOW(IN id_cow INT)
BEGIN
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
	BEGIN
		GET DIAGNOSTICS CONDITION 1 @p1 = RETURNED_SQLSTATE, @p2 = MESSAGE_TEXT;
	...
	START TRANSACTION;
    BEGIN
        IF NOT DEADCOW_EXISTS(id_cow) THEN
			SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'BROTHER LA VACA NO ESTA REPORTADA COMO MUERTA';
        END IF;
		SELECT * FROM descriptive_deadcow_view d WHERE d.id_cow = id_cow;
    COMMIT;
    END;
END;;
DELIMITER ;
