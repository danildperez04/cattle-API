...
BEGIN
    ...
    BEGIN
        ...
        SELECT id_breed FROM breedcow bc WHERE bc.id_cow = id_cow INTO @idAlreadyExist;
...
DROP PROCEDURE IF EXISTS UPDATE_BREEDCOW; -- ACTUALIZA UN NUEVO REGISTRO PARA LAS VACAS CON RAZA
DELIMITER ;;
CREATE PROCEDURE UPDATE_BREEDCOW(IN id_breed INT, IN id_cow INT)
BEGIN
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
	BEGIN
		GET DIAGNOSTICS CONDITION 1 @p1 = RETURNED_SQLSTATE, @p2 = MESSAGE_TEXT;
    ...
    BEGIN
		...
		UPDATE breedcow bc SET bc.id_breed = id_breed WHERE bc.id_cow = id_cow;
        COMMIT;
    END;
END;;
DELIMITER ;
